...
{acl_GATBPS_CHECK}(<message>, <name>, <code>, [<precondition>])
----
...
* If `<precondition>` is given, then the behavior is as if `:bool` was
specified at the end of `<name>`, and `<precondition>` will be
interpreted as a boolean expression.
If the result of `<precondition>` is false, then `g_cv_<name>` will be
set to `no` instead of executing `<code>` to set it.
`<precondition>` may use variable names, literal ``1```'s and ``0```'s,
the `!`, `&&`, and `||` operators, parentheses for precedence, and
space, tab, and newline characters for separation.
Each variable should already be set to `1` or `0`.
