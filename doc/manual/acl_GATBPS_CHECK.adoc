...
* {empty}
`<name>_is_cached` is set to `1` or `0` to indicate whether `<name>` is
cached.

* {empty}
`<name>_is_cached_sh` is set to `:` or `false` corresponding to
`<name>_is_cached`.
...
** If `gatbps_cv_<name>` is set to `yes` or `no`, then all of the
following occur:
...
** If `gatbps_cv_<name>` is set to something other than `yes` or `no`,
then `<name>_sh` is set to `false`.
