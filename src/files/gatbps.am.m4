...
## code is idempotent with respect to running multiple times on the same
## directory.
##
## Note that we run mkdir -p on all directories in uninstall-prehook,
## only to delete them afterward in uninstall-hook. This is because the
## uninstallation code generated by Automake sometimes gets angry about
