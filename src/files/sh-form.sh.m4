...
#
# Note that the above code contains escape characters, i.e. bytes with
...
# but would produce inconsistent results across systems.
#
...
# name that resolves to the best available version of P. The variable
# starts off as empty and is lazily set using the following code just
# before each point where the value is needed.
#
# For awk:
#
#       case "${awk_auto}" in
...
#
# For each program P of interest, the P variable holds the command name
# to be used to run P. The variable starts off with a value selected by
# the configure script and is updated by the P environment variable and
# instances of the --P=V option. If the variable is set to 'auto', then
# it is adjusted to the value of the P_auto variable.
#
